...
def generate_calendar() -> dict[str, list[str]]:
    """Generates a random calendar for Apeksha for the next 7 days."""
    calendar = {}
    today = date.today()
    ...
    print("Apeksha's calendar:", calendar)

    return calendar


APEKSHA_CALENDAR = generate_calendar()


def get_availability(start_date: str, end_date: str) -> str:
    """
    Checks Apeksha's availability for a given date range.
    ...
    Returns:
        A string listing Apeksha's available times for that date range.
    """
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = APEKSHA_CALENDAR.get(date_str, [])
            if available_slots:
                availability = f"On {date_str}, Apeksha is available at: {', '.join(available_slots)}."
                results.append(availability)
            else:
                results.append(f"Apeksha is not available on {date_str}.")
...
def create_agent() -> LlmAgent:
    return LlmAgent(
        ...
        name="apeksha_agent",
        instruction="""
         **Role:** You are Apeksha's personal scheduling assistant. 
            Your sole responsibility is to manage her calendar and respond to inquiries 
            about her availability for pickleball.

            **Core Directives:**

            *   **Check Availability:** Use the `get_apeksha_availability` tool to determine 
                    if Apeksha is free on a requested date or over a range of dates. 
