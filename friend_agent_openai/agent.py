...
def generate_calendar() -> dict[str, list[str]]:
    """Generates a random calendar for Darshana for the next 7 days."""
    calendar = {}
    today = date.today()
    ...
    print("Darshana's calendar:", calendar)

    return calendar


DARSHANA_CALENDAR = generate_calendar()

@function_tool
def get_availability(start_date: str, end_date: str) -> str:
    """
    Checks Darshana's availability for a given date range.
    ...
    Returns:
        A string listing Darshana's available times for that date range.
    """
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = DARSHANA_CALENDAR.get(date_str, [])
            if available_slots:
                availability = f"On {date_str}, Darshana is available at: {', '.join(available_slots)}."
                results.append(availability)
            else:
                results.append(f"Darshana is not available on {date_str}.")
...
def create_agent() -> Agent:
    return Agent(
        ...
        instructions="""
         **Role:** You are Darshana's personal scheduling assistant. 
            ...
            *   **Check Availability:** Use the `get_availability` tool to determine 
                    if Darshana is free on a requested date or over a range of dates. 
